...
This article is based on the [JSON‑RPC 2.0 Specification](https://www.jsonrpc.org/specification)
by the JSON‑RPC Working Group, Copyright (C) 2007–2010 by the JSON‑RPC Working
Group, used under the permissions granted in the specification.
...
- `params`: Structured value that holds parameter values for invocation
  (Array or Object, optional).
- `id`: Identifier defined by the client (String, Number, or Null, optional).
  If omitted, the message is assumed to be a notification. Otherwise, the server
...
client's lack of interest in the Response object. The server must not reply
to a notification.

### Parameters Structure

Parameters must be provided as structured values, either by position through
an array or by name through an object.

## Response Object

The response is expressed as a single JSON object with the following members.

- `jsonrpc`: Version of the JSON-RPC protocol (String, required, must
  be `"2.0"`).
...
A response object should exist for each request object except for notification
requests.

The responses may be returned in any order in the responses array.
The server must not return an empty array and should return nothing at all for
batches containing only notifications.
